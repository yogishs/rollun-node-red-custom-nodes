...
module.exports = function (RED) {
  ...
  function getTzValue(msg, type, value) {
    if (!value) {
      return '';
    }
    let zone;
    switch (type) {
      case 'msg':
        zone = RED.util.getMessageProperty(msg, value)
        break
        break
      case 'str':
        zone = value.trim()
      ...
      default:
        zone = 'Etc/UTC'
        node.warn('Unrecognised Input Type, ' + type + '. Zone set to Etc/UTC.')
    }
    return zone;
  }
  ...
  function nodeDefinition(config) {
    ...
    node.on('input', function (msg, send, done) {
      ...
      node.inTz = getTzValue(msg, config.inTzType, config.inTz);
      node.outTz = getTzValue(msg, config.outTzType, config.outTz);
